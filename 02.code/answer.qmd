...
ready_outcome_data <- function(df_outcome_tidy) {
    ...
    df_output <- df_outcome_tidy |> 
        dplyr::mutate(
            ...
            gradrate = round(tot4yrgrads / totcohortsize, digits = 3),
            men_gradrate = round(m_4yrgrads / m_cohortsize, digits = 3)
        ) |> 
        tidyr::drop_na(gradrate)
...
## (e) covariates_ready
1. outcomeやsemester_dummyのデータの期間と合わせてください。
1. outcome_dataに含まれるunitidを特定し、covariatesのunitidを限定してください。
...
create_table <- function(df_master) {

    df_kable_based <- df_master %>%
        dplyr::summarise(
            dplyr::across(-c(unitid, year, instnm, semester, quarter), list(
